...
function config_url {
    local params="distro=${distro}&codename=${codename}&version=${version}&arch=${arch}"
    test ! -z "$component" && {
        params="${params}&component=${component}"
    }
    echo "https://dl.cloudsmith.io/public/balena/etcher/config.deb.txt?$params" | sed 's/ /%20/g'
}
...
function import_gpg_key {
    local text="Importing 'balena/etcher' repository GPG keys ..."
    echo_running "$text"

    local gpg_keyring_path="/usr/share/keyrings/balena-etcher-archive-keyring.gpg"
    
    curl -1sLf "https://dl.cloudsmith.io/public/balena/etcher/gpg.70528471AFF9A051.key" | gpg --dearmor >> $gpg_keyring_path
    ...
    echo_okfail "Checking for apt signed-by key support ..." || {
        mv ${gpg_keyring_path} /etc/apt/trusted.gpg.d/balena-etcher.gpg
    }
